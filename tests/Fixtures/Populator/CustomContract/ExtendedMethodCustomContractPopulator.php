<?php
declare(strict_types=1);

namespace Neusta\ConverterBundle\Tests\Fixtures\Populator\CustomContract;

use Neusta\ConverterBundle\Converter\Context\GenericContext;
use Neusta\ConverterBundle\Tests\Fixtures\Model\Source\User;
use Neusta\ConverterBundle\Tests\Fixtures\Model\Target\Person;

final class ExtendedMethodCustomContractPopulator implements ExtendedMethodCustomContractInterface
{
    public function populateName(User $user, Person $person, ?GenericContext $context = null): void
    {
        $person->setFullName($user->getFirstname());
    }

    public function populateAge(User $user, Person $person): void
    {
        $person->setAge($user->getAgeInYears());
    }
}
